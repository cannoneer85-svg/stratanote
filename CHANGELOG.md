...
## [1.16.0] - 2026-08-04
### Document Ownership Transfer for Administrators

- Added PUT /api/notes/owner API endpoint enabling System Administrators to reassign document or folder ownership (created_by) to any registered user or back to \
- External
- System\.
- Added recursive ownership transfer option for directories, updating all nested subfiles and subfolders in a single action.
- Created ChangeOwnerModal modal dialog with user selection dropdown, loading indicators, and safe error handling.
- Integrated \
- Change
- Owner\ actions in the sidebar explorer hover controls (for Admins), Version History drawer header, and editor footer.
- Optimized sidebar item hover controls with high-contrast icon styling across all themes (Dark Purple, Soft Light, Clean Light, Nord Dark).
- Added real-time WebSocket broadcasting (note:owner_changed) to synchronize document permissions dynamically across all active client sessions.
- Updated user guide documentation (docs/user-guides/collaboration.mdx).

## [1.15.0] - 2026-08-04
### PlantUML Rendering Engine and Appearance Settings

- Added PlantUML rendering engine with 100% offline Java fallback (plantuml.jar) and interactive pan & zoom modal.
- Introduced Appearance settings tab (Внешний вид) accessible to all user roles (Admin, Editor, Viewer).
- Added themes: Soft Light Purple (Светлая лаванда), Clean Light, Nord Dark, and classic Dark Purple.
- Per-user theme preferences stored in browser storage (stratanote_theme_<username>), ensuring authentication screen always stays in default dark-purple style.
- Added quick theme toggle button (Sun/Moon) in sidebar footer.
- Optimized CSS variables and border calculations for Tailwind compatibility.
