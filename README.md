...
*   `VAULT_PATH` — путь к папке Obsidian-хранилища, которую приложение должно обслуживать. По умолчанию используется родительский каталог папки `_app` (корень репозитория).
...
2.  **Подключение диска (Volume)**:
    *   Создайте новый **Volume** (например, `obsidian-collab-volume` размером 5 GB) в панели вашего проекта Railway.
