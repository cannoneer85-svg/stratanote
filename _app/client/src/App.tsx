 ...
 * Orchestrates global states, socket connections, view routing, notifications, and modal managers.
 */
...
import { forceDefaultTheme, initUserTheme } from './utils/theme';
import { 
 ...
 * and coordinates interaction between {@link Sidebar}, {@link Editor}, {@link GraphView}, and admin panels.
 */
export default function App() {
  ...
  const [currentUser, setCurrentUser] = useState<any>(() => {
    ...
    if (saved) {
      ...
      // Immediately restore the user's theme on page load
      if (user && user.username) {
        initUserTheme(user.username);
      }
      return user;
    }
    return null;
  ...
  // Note templates folder (statically set to 'Templates' for sidebar and system-wide exclusion)
  ...
  const [changeOwnerTarget, setChangeOwnerTarget] = useState<{
    relative_path: string;
    title: string;
  ...
  const handleOwnerChanged = (relPath: string, newOwner: string, recursive: boolean) => {
    setNotes(prevNotes => prevNotes.map(n => {
      if (n.relative_path === relPath || (recursive && n.relative_path.startsWith(relPath + '/'))) {
        return { ...n, created_by: newOwner };
      }
      return n;
    }));
  };
  ...
  const noteTabsContainerRef = useRef<HTMLDivElement>(null);

  // Horizontal scroll via mouse wheel for opened note tabs
  useEffect(() => {
    const container = noteTabsContainerRef.current;
    if (!container) return;
  ...
  // Load system default language on mount
  useEffect(() => {
    fetch('/api/auth/system-language')
      .then(res => res.json())
      .then(data => {
        if (data && data.language) {
          // Only apply if user is NOT logged in (if logged in, JWT settings will override it)
          if (!localStorage.getItem('token')) {
            setLang(data.language);
            localStorage.setItem('stratanote_lang', data.language);
          }
        }
      })
      .catch(err => console.error('Failed to load default system language:', err));
  }, []);
  ...
  const handleSetLang = (newLang: Lang) => {
    ...
    if (token) {
      ...
      .then(res => {
        if (!res.ok) console.error('Failed to save language setting to server');
      })
      .catch(err => console.error('Failed to save language to server:', err));
    }
  };
  ...
  useEffect(() => {
    ...
    // Load user settings from server (including custom language preferences)
    fetch('/api/auth/settings', {
      headers: { 'Authorization': `Bearer ${token}` }
    })
    .then(res => res.json())
    ...
    .catch(err => console.error('Failed to load user settings from server:', err));
    ...
    socketInstance.on('notes:updated', () => {
      loadNotes();
    });

    socketInstance.on('note:owner_changed', ({ relative_path, created_by, recursive }: { relative_path: string; created_by: string; recursive?: boolean }) => {
      handleOwnerChanged(relative_path, created_by, !!recursive);
    });
  ...
  const openNote = async (path: string) => {
    ...
    if (noteContents[path] === undefined) {
      try {
        ...
        if (res.status === 403) {
          alert(t('settings_templates_access_denied', lang));
          setOpenedTabs(prev => prev.filter(t => t !== path));
          setActiveNotePath(null);
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem('stratanote_lang');
    setToken(null);
    setCurrentUser(null);
    setOpenedTabs([]);
    setActiveNotePath(null);
    ...
    // Reset theme to default dark-purple for auth screen
    forceDefaultTheme();

    // Reset language to default system language
    fetch('/api/auth/system-language')
      .then(res => res.json())
      .then(data => {
        if (data && data.language) {
      ...
      .catch(err => console.error('Failed to load default system language on logout:', err));
  };
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-background relative">
      ...
      >
        <div className="flex-1 h-full w-full min-w-0">
          <Sidebar
            ...
            onOpenSearch={(initQuery) => {
              setSearchInitialQuery(initQuery || '');
            ...
            onChangeOwner={(item) => {
              setChangeOwnerTarget({
                ...
                title: item.title,
                current_owner: item.created_by || 'Внешняя система',
      ...
      <div className="flex-1 h-full flex flex-col overflow-hidden min-w-0">
        ...
        <div className="h-12 border-b border-border bg-background-panel flex items-center justify-between px-4 select-none">
          <div 
            ref={noteTabsContainerRef}
            className="flex items-center space-x-1.5 overflow-x-auto scrollbar-none flex-1 pr-4"
          >
            {/* Sidebar toggle button */}
            <button
              onClick={() => setSidebarOpen(prev => !prev)}
              className="p-1.5 hover:bg-background-hover rounded-lg text-text-muted hover:text-text transition-colors cursor-pointer mr-1"
              title={sidebarOpen 
              ...
              return (
                <div
                  ...
                  className={`flex items-center space-x-2 px-3 py-1.5 rounded-t-lg border-t border-x text-xs cursor-pointer transition-colors max-w-[150px] truncate ${
                    isActive 
                      ? 'bg-background-editor border-border text-text font-semibold shadow-inner' 
                      : 'border-transparent text-text-muted hover:bg-background-hover hover:text-text'
                  }`}
                >
                  ...
                  <button
                    ...
                    className="p-0.5 hover:bg-background-hover hover:text-text rounded text-text-disabled transition-colors"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </div>
          ...
          <div className="flex items-center space-x-2">
            <div className="flex rounded-lg p-0.5 bg-background-editor">
              <button
                onClick={() => setActiveTab('editor')}
                className={`p-1 px-2.5 sm:px-3.5 rounded text-xs flex items-center space-x-1.5 transition-all cursor-pointer ${
                  activeTab === 'editor' ? 'bg-primary text-white font-medium shadow-sm' : 'text-text-muted hover:text-text'
                }`}
              >
              ...
              <button
                ...
                className={`p-1 px-2.5 sm:px-3.5 rounded text-xs flex items-center space-x-1.5 transition-all cursor-pointer ${
                  activeTab === 'graph' ? 'bg-primary text-white font-medium shadow-sm' : 'text-text-muted hover:text-text'
                }`}
              >
            ...
            {activeNotePath && activeTab === 'editor' && (
              <button
                ...
                className={`p-1.5 border rounded-lg text-text-muted hover:text-text cursor-pointer transition-colors ${
                  historyOpen ? 'bg-primary/20 border-primary/40 text-primary' : 'bg-background-editor border-border'
                }`}
                ...
                <div className="w-full h-full relative">
                  {selectedHistoryItem ? (
                    ...
                    <Editor
                      ...
                      onChangeOwner={(path) => {
                        const target = notes.find(n => n.relative_path === path);
                        if (target) {
                          setChangeOwnerTarget({
                            relative_path: target.relative_path,
                            title: target.title,
                            current_owner: target.created_by || 'Внешняя система',
              ...
              <div className="w-80 h-full bg-background-panel border-l border-white/5 flex flex-col overflow-hidden animate-slide-in shrink-0">
                <div className="p-4 border-b border-white/5 flex items-center justify-between bg-black/10 select-none">
                  <div>
                    ...
                    <div className="text-[10px] text-text-disabled mt-0.5 flex items-center space-x-1.5">
                      <span>{t('history_owner', lang)}: <span className="text-primary font-semibold">{noteCreator}</span></span>
                      {currentUser?.role === 'Admin' && currentNote && (
                        <button
                          onClick={() => setChangeOwnerTarget({
                            relative_path: currentNote.relative_path,
                            title: currentNote.title,
                            current_owner: currentNote.created_by || 'Внешняя система',
                            is_directory: currentNote.is_directory
                          })}
                          className="px-1.5 py-0.5 text-[9px] bg-primary/10 hover:bg-primary/20 text-primary border border-primary/30 rounded font-semibold transition-colors cursor-pointer"
                ...
                <div className="flex-1 overflow-y-auto p-3 space-y-2 select-none">
                  {historyList.length === 0 ? (
                    ...
                    historyList.map((item) => {
                      ...
                      const authorDisplay = item.author_name === 'Внешняя система' || item.author_name === 'system' 
                        ? t('system_external', lang) 
                        : item.author_name;
                      return (
                        <div
                        ...
                        >
                          <div className="flex items-center justify-between mb-1 gap-2">
                            <div className="flex items-center min-w-0">
                              ...
                              {isCurrent && (
                                <span className="ml-1.5 text-[9px] text-green-400 bg-green-500/10 border border-green-500/20 px-1.5 rounded-full font-bold shrink-0">
                                  {lang === 'en' ? 'Current' : 'Текущая'}
                                </span>
                              )}
                            </div>
                            <span 
                              className="text-[9px] text-primary bg-primary/10 border border-primary/20 px-1.5 rounded-full font-bold max-w-[100px] truncate shrink-0 inline-block"
                              title={authorDisplay}
                            >
                              {authorDisplay}
                            </span>
      ...
      <SettingsPanel
        ...
        notes={notes}
        templatesFolder={currentUser ? `Templates/${currentUser.username}` : ''}
        onOpenNote={openNote}
      />
      ...
      {changeOwnerTarget && token && (
        <ChangeOwnerModal
          isOpen={!!changeOwnerTarget}
          onClose={() => setChangeOwnerTarget(null)}
          relativePath={changeOwnerTarget.relative_path}
          title={changeOwnerTarget.title}
          currentOwner={changeOwnerTarget.current_owner}
          isDirectory={changeOwnerTarget.is_directory}
          lang={lang}
          token={token}
