/**
 * @module Auth
 * Provides authentication views, token storage handlers, and signup options.
 */
...
/**
 * Properties for the {@link Auth} component.
 */
interface AuthProps {
...
/**
 * User authentication and registration panel component.
 * Displays login forms or signup request forms. Handles JWT session generation
 * on the backend and queries the server for version tags to display in the footer.
 */
export const Auth: React.FC<AuthProps> = ({ onLoginSuccess, lang }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errMsg === 'Awaiting administrator approval' || errMsg === 'Пользователь не найден или ожидает одобрения') {
          errMsg = t('auth_approval_pending', lang);
        } else if (errMsg === 'Invalid username or password' || errMsg === 'Неверный пароль' || errMsg === 'Пользователь не найден') {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md">
      <div className="w-full max-w-md p-8 rounded-2xl bg-background-panel border border-primary/50 glow-active shadow-2xl text-center mx-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 bg-black/30 border border-primary/15 rounded-xl text-text placeholder-text-disabled focus:outline-none focus:border-primary/50 transition-colors"
              required
            />
          </div>
