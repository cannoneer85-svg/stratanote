...
export const GraphView: React.FC<GraphViewProps> = ({
  ...
  const drawNode = (node: any, ctx: CanvasRenderingContext2D, globalScale: number) => {
    ...
    if (isActive) {
      ...
      ctx.fillStyle = isHovered ? '#ffffff' : '#a78bfa';
    }

    // Draw circle (for tags we can draw a circle, or a small hexagon/square, but circle is very neat)
    ctx.beginPath();
    ...
    // Node Text Label (render tags labels more aggressively so they act as anchors)
    ...
    if (showLabel) {
      ...
      // Label opacity & style
      let textStyle = node.isTag ? '#22d3ee' : 'rgba(224, 224, 224, 0.8)';
