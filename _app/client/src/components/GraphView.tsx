/**
 * @module GraphView
 * Interactive 2D visualization of folders structure, links connections, and vector similarity relations.
 */
...
/**
 * Properties for the {@link GraphView} component.
 */
...
export const GraphView: React.FC<GraphViewProps> = ({
  ...
  const canvasBgColor = useMemo(() => {
    if (currentTheme === 'light-purple') return '#ffffff';
    if (currentTheme === 'light-clean') return '#ffffff';
    if (currentTheme === 'nord-dark') return '#3b4252';
    return '#181818';
  }, [currentTheme]);
  ...
  // Dynamically extract all display folders present in the data (excluding tags)
  ...
  // Filter nodes based on folder exclusions and tag visibility
  ...
  }, [graphData.nodes, excludedFolders, showTags]);

  // Filter links dynamically based on user selections AND folder exclusions
  const filteredLinks = useMemo(() => {
    const nodeIds = new Set(filteredNodes.map(n => n.id));
    
    // 1. Separate wiki links, semantic candidates, and tag links
    const wikiLinks: any[] = [];
    ...
    return [...wikiLinks, ...Array.from(topSemanticLinks), ...tagLinks];
  }, [graphData.links, filteredNodes, showWikiLinks, showSemanticLinks, showTags, similarityThreshold]);
  ...
  const visibleDegrees = useMemo(() => {
    ...
    filteredLinks.forEach((l: any) => {
      ...
      if (l.isSemantic) {
        ...
        // Tag links count 0.5 for nodes (since tag nodes are central hubs)
        degrees[sourceId] = (degrees[sourceId] || 0) + 0.5;
        degrees[targetId] = (degrees[targetId] || 0) + 0.5;
      } else {
  ...
  // Handle graph physics forces configuration on load and node/link changes
  useEffect(() => {
    if (graphRef.current) {
      const nodeCount = graphData.nodes.length;
      const chargeStrength = nodeCount <= 2 ? -400 : nodeCount <= 5 ? -300 : -250;

      // 1. Configure repulsive force (charge)
      graphRef.current.d3Force('charge')
        .strength(chargeStrength)
        .distanceMax(350);

      // 2. Configure link distances and strengths dynamically
      graphRef.current.d3Force('link')
        .distance((link: any) => {
          if (link.isTagLink) return 60;
          if (link.isSemantic) return 110;
          return 80;
        })
        .strength((link: any) => {
          if (link.isSemantic) return 0.04;
          if (link.isTagLink) return 0.12;
          return 0.22;
        });

      // 3. Add collision force to prevent nodes from overlapping
      graphRef.current.d3Force('collide', forceCollide((node: any) => {
        // Tag nodes are slightly larger, regular nodes are smaller
        return node.isTag ? 14 : 8;
      }).strength(0.8));
      
      // Reheat the force simulation to let the nodes layout beautifully
      graphRef.current.d3ReheatSimulation();
    }
  }, [graphData]);

  // Unfreeze nodes and trigger simulation reheat when layers or filters change
  useEffect(() => {
  ...
  }, [showWikiLinks, showSemanticLinks, showTags, similarityThreshold, excludedFolders]);
  ...
  const drawNode = (node: any, ctx: CanvasRenderingContext2D, globalScale: number) => {
    ...
    const baseRadius = node.isTag ? 5.5 : Math.max(3.5, 3.5 + Math.log1p(degreeCount) * 2.2);
    let radius = baseRadius;

    // Draw shadow glow for active/searched/hovered/tag
    if (isActive || isHighlighted || isHovered) {
      ctx.shadowColor = isActive ? '#9d4edd' : isHighlighted ? '#f59e0b' : node.isTag ? '#06b6d4' : '#a78bfa';
      ctx.shadowBlur = 15;
    ...
    // Set colors
    if (isActive) {
      ctx.fillStyle = isLight ? '#7c3aed' : '#9d4edd'; // Purple active
    } else if (isHighlighted) {
      ctx.fillStyle = '#f59e0b'; // Amber search match
    } else if (hoverNode && !isHovered && !isNeighbor) {
      ctx.fillStyle = isLight ? 'rgba(0, 0, 0, 0.1)' : 'rgba(255, 255, 255, 0.15)'; // Faded out
    } else if (node.isTag) {
      ctx.fillStyle = '#06b6d4'; // Cyan for tags
    } else {
      ctx.fillStyle = isHovered 
        ? (isLight ? '#1e1b26' : '#ffffff') 
    ...
    // Node Text Label
    const showLabel = globalScale > 0.85 || isActive || isHighlighted || isHovered || isNeighbor || node.isTag;
    if (showLabel) {
      const label = node.name;
      const fontSize = Math.min(11, Math.max(4, 9 / Math.sqrt(globalScale)));
      ctx.font = node.isTag ? `bold ${fontSize}px Outfit, Inter, sans-serif` : `${fontSize}px Outfit, Inter, sans-serif`;
      ctx.textAlign = 'center';
      ctx.textBaseline = 'top';

      // Label opacity & style
      let textStyle = node.isTag 
        ? (isLight ? '#0891b2' : '#22d3ee') 
        : (isLight ? 'rgba(30, 27, 38, 0.85)' : 'rgba(224, 224, 224, 0.8)');
      if (isActive) textStyle = isLight ? '#6d28d9' : '#ffffff';
      else if (isHighlighted) textStyle = '#d97706';
      else if (hoverNode && !isHovered && !isNeighbor) textStyle = isLight ? 'rgba(0, 0, 0, 0.15)' : 'rgba(255, 255, 255, 0.1)';
  ...
  return (
    ...
    `}>
      ...
      <div className="absolute top-4 left-4 z-10 flex flex-col space-y-2">
        ...
        <div className="bg-black/60 backdrop-blur-md border border-white/5 p-3 rounded-lg flex flex-col space-y-2 w-64 text-xs text-text">
          ...
          <label className="flex items-center space-x-2 cursor-pointer select-none">
            ...
            />
            <span>{t('graph_toggle_tags', lang)}</span>
          </label>
      ...
      <div className="flex-1 w-full h-full relative cursor-grab active:cursor-grabbing select-none">
        {isLoading ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center text-text-muted text-sm space-y-3">
            <div className="w-8 h-8 border-2 border-primary border-t-transparent rounded-full animate-spin"></div>
            <span>{lang === 'en' ? 'Loading graph data...' : 'Загрузка данных графа...'}</span>
          </div>
          ...
          <ForceGraph2D
            ...
            linkColor={(link: any) => {
              ...
              if (link.isTagLink) {
                if (hoverNode) {
                  return isHovered ? '#22d3ee' : 'rgba(255, 255, 255, 0.02)';
                }
                if (activeNotePath) {
                  return isActive ? '#06b6d4' : 'rgba(255, 255, 255, 0.04)';
                }
                return 'rgba(6, 182, 212, 0.25)'; // Cyan semi-transparent
              }
            ...
            linkDirectionalArrowLength={(link: any) => {
              if (link.isSemantic || link.isTagLink) return 0; // semantic links and tags links are symmetric and undirected
              const sourceId = typeof link.source === 'object' ? link.source.id : link.source;
              const targetId = typeof link.target === 'object' ? link.target.id : link.target;
              const isHovered = hoverNode && (hoverNode.id === sourceId || hoverNode.id === targetId);
            ...
            linkDirectionalArrowColor={(link: any) => {
              if (link.isSemantic || link.isTagLink) return 'transparent';
              const sourceId = typeof link.source === 'object' ? link.source.id : link.source;
              const targetId = typeof link.target === 'object' ? link.target.id : link.target;
              const isHovered = hoverNode && (hoverNode.id === sourceId || hoverNode.id === targetId);
              const isActive = activeNotePath && (activeNotePath === sourceId || activeNotePath === targetId);
            ...
            linkDirectionalParticles={(link: any) => {
              ...
              return (link.isSemantic || link.isTagLink) ? 0 : 0.6; // No default flow for semantic links & tags to prevent clutter
            }}
            ...
            nodeLabel={(node: any) => {
              if (node.isTag) {
                let noteConnectionsCount = 0;
                filteredLinks.forEach((l: any) => {
                  const sId = typeof l.source === 'object' ? l.source.id : l.source;
                  const tId = typeof l.target === 'object' ? l.target.id : l.target;
                  if (sId === node.id || tId === node.id) {
                    noteConnectionsCount++;
                  }
                });
                return `<div style="background: rgba(18,18,18,0.95); border: 1px solid rgba(6,182,212,0.4); border-radius: 6px; padding: 8px 12px; color: #f3f4f6; font-size: 11px; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1), 0 2px 4px -2px rgb(0 0 0 / 0.1); font-family: Inter, sans-serif; line-height: 1.4; pointer-events: none;">
                  <div style="font-weight: 700; color: #22d3ee; margin-bottom: 2px; font-size: 12px;">${node.name}</div>
                  <div style="color: #9ca3af; font-size: 9px; margin-bottom: 6px;">${lang === 'en' ? 'Tag grouping node' : 'Тег-группиратор'}</div>
                  <div style="border-top: 1px solid rgba(255,255,255,0.08); padding-top: 6px; margin-top: 6px;">
                    ${lang === 'en' ? 'Linked notes:' : 'Связано заметок:'} <span style="font-weight: 700; color: #fff;">${noteConnectionsCount}</span>
                  </div>
                </div>`;
              }
              ...
              const totalLinks = wikiLinksCount + semanticLinksCount + tagLinksCount;
              const filename = node.id.split('/').pop() || node.id;
              const title = filename.endsWith('.md') ? filename.slice(0, -3) : filename;

              let linksHtml = `
                <div>${lang === 'en' ? 'Total links:' : 'Всего связей:'} <span style="font-weight: 700; color: #fff;">${totalLinks}</span></div>
                <div style="color: #a78bfa;">• ${lang === 'en' ? 'Wiki links:' : 'Вики-ссылки:'} <span style="font-weight: 700;">${wikiLinksCount}</span></div>
                <div style="color: #c084fc;">• ${lang === 'en' ? 'Semantic links:' : 'Логические связи:'} <span style="font-weight: 700;">${semanticLinksCount}</span></div>
              `;
              if (showTags) {
                linksHtml += `<div style="color: #22d3ee;">• ${lang === 'en' ? 'Tags:' : 'Связанные теги:'} <span style="font-weight: 700;">${tagLinksCount}</span></div>`;
              }
            ...
            linkLabel={(link: any) => {
              if (link.isSemantic) {
                return `<div style="background: rgba(18,18,18,0.95); border: 1px solid rgba(168,85,247,0.4); border-radius: 6px; padding: 6px 10px; color: #f3f4f6; font-size: 11px; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1), 0 2px 4px -2px rgb(0 0 0 / 0.1); font-family: Inter, sans-serif; pointer-events: none;">
                  <span style="color: #c084fc; font-weight: 600;">${lang === 'en' ? 'Semantic correlation link' : 'Логическая семантическая связь'}</span><br/>
                  ${lang === 'en' ? 'Correlation score:' : 'Сходство текстов:'} <span style="font-family: monospace; color: #a855f7; font-weight: 700;">${(link.similarity * 100).toFixed(0)}%</span>
                </div>`;
              }
              return '';
            }}
            ...
            onNodeClick={(node: any) => {
              if (node.isTag) {
                if (onOpenSearch) {
                  onOpenSearch(`#${node.name.replace('#', '')}`);
                }
              } else {
