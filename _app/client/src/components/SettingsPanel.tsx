 ...
 * Administration settings panel displaying localized configuration forms, sync triggers, and media assets explorer.
 */
import React, { useState, useEffect, useRef, useMemo } from 'react';
import { X, Upload, UserPlus, Trash2, AlertTriangle, Check, Users, ShieldAlert, FolderOpen, Edit2, Image, Search, Info, RefreshCw, Globe, Play, ExternalLink, Sparkles, CheckCheck, LayoutTemplate, Palette, Sun, Moon } from 'lucide-react';
...
/**
 * Properties for the {@link SettingsPanel} component.
 */
...
/**
 * System configuration settings and administrative dashboard panel component.
 * Organizes management tasks across multiple settings tabs:
 * - General: Localization setup.
 * - Import & Upload: ZIP backups extraction and merge modes.
 * - Users: Approvals, registrations, and access control role selection.
 * - Media Library: File grid explorer, size analysis, and orphaned file filters.
 * - Synchronization: API token generation and vector search indices rebuilding.
 * - Trash Bin: Recovering notes and historical version logs.
 * - System: Checking update versions, showing system environment diagnostics, and link checkers.
 */
export const SettingsPanel: React.FC<SettingsPanelProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'general' | 'appearance' | 'import' | 'users' | 'media' | 'about' | 'sync' | 'trash' | 'templates'>('general');
  ...
  // Media Pagination State & Scroll Ref
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [mediaTypeFilter, mediaSearchQuery, activeTab, isOpen]);
  ...
  const [templatesDateFormat, setTemplatesDateFormat] = useState<string>('YYYY-MM-DD');
  const [templatesTimeFormat, setTemplatesTimeFormat] = useState<string>('HH:mm');
  const [templatesStatus, setTemplatesStatus] = useState<{ type: 'success' | 'error'; message: string } | null>(null);

  // Load template settings when panel opens
  useEffect(() => {
    if (isOpen && token) {
      ...
      .then(data => {
        if (data) {
          if (data.templates_date_format) setTemplatesDateFormat(data.templates_date_format);
          if (data.templates_time_format) setTemplatesTimeFormat(data.templates_time_format);
        }
      })
      .catch(err => console.error('Failed to load template settings:', err));
    }
  }, [isOpen, token]);

  // folders list is no longer needed since templates folder is statically set to Templates/<username>

  const templatesList = useMemo(() => {
    if (!templatesFolder) return [];
    const prefix = templatesFolder.endsWith('/') ? templatesFolder : templatesFolder + '/';
    return notes.filter(n => !n.is_directory && n.relative_path.startsWith(prefix));
  }, [notes, templatesFolder]);

  const handleSaveTemplatesSettings = async () => {
    try {
      const res = await fetch('/api/auth/settings', {
        ...
        body: JSON.stringify({
          templates_date_format: templatesDateFormat,
          templates_time_format: templatesTimeFormat
        })
      });
      if (!res.ok) throw new Error();
      
      setTemplatesStatus({ type: 'success', message: t('settings_templates_save_success', lang) });
      setTimeout(() => setTemplatesStatus(null), 3000);
    } catch (err) {
      setTemplatesStatus({ type: 'error', message: t('settings_templates_save_failed', lang) });
      setTimeout(() => setTemplatesStatus(null), 3000);
    }
  };
  ...
  const handleCreateTemplate = async () => {
    if (!templatesFolder) return;
    const name = prompt(t('settings_templates_create_prompt', lang));
    if (!name) return;
    const trimmed = name.trim();
    if (!trimmed) {
      alert(t('settings_templates_create_err_empty', lang));
      return;
    }
    
    const fileName = trimmed.endsWith('.md') ? trimmed : `${trimmed}.md`;
    const relativePath = `${templatesFolder}/${fileName}`;
    ...
    try {
      ...
      alert(t('settings_templates_create_failed', lang) + ': ' + err.message);
    }
  };
  ...
  const PAGE_SIZE = 60;
  const totalPages = Math.ceil(filteredMediaFiles.length / PAGE_SIZE);
  const validCurrentPage = Math.max(1, Math.min(currentPage, totalPages || 1));
  const paginatedMediaFiles = filteredMediaFiles.slice(
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in select-none">
      <div className="relative w-full max-w-3xl h-[600px] max-h-[85vh] flex flex-col bg-background-panel border border-white/10 rounded-2xl overflow-hidden shadow-glass animate-scale-up">
        ...
        <div 
          ...
          className="flex border-b border-white/5 bg-black/10 px-6 py-2 space-x-2 overflow-x-auto scrollbar-none flex-nowrap shrink-0"
        >
          <button
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-semibold flex items-center space-x-2 transition-all cursor-pointer shrink-0 ${
              activeTab === 'appearance' ? 'bg-primary text-white shadow-glow' : 'text-text-muted hover:text-white'
            }`}
          >
            <Palette className="w-3.5 h-3.5" />
            <span>{t('settings_tab_appearance', lang)}</span>
          </button>
          ...
          {currentUser.role !== 'Viewer' && (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-xs font-semibold flex items-center space-x-2 transition-all cursor-pointer shrink-0 ${
                activeTab === 'templates' ? 'bg-primary text-white shadow-glow' : 'text-text-muted hover:text-white'
              }`}
            >
              <LayoutTemplate className="w-3.5 h-3.5" />
              <span>{t('settings_tab_templates', lang)}</span>
            </button>
          )}
          {currentUser.role === 'Admin' && (
            <button
        ...
        {/* Main Content Area */}
        <div ref={contentAreaRef} className="flex-1 overflow-y-auto p-6 space-y-6">
          {activeTab === 'general' ? (
            ...
            <div className="space-y-6">
              {/* Theme Settings Card */}
              <div className="p-5 rounded-2xl bg-background-panel border border-border space-y-4 text-left">
                <div className="flex items-center space-x-2 text-primary">
                  <Palette className="w-5 h-5" />
                  <h3 className="text-sm font-bold text-text uppercase tracking-wider">{t('settings_appearance_title', lang)}</h3>
                </div>
                <p className="text-xs text-text-muted">{t('settings_appearance_subtitle', lang)}</p>
                  ...
                  {THEMES.map((themeItem) => {
                    ...
                    return (
                      <div
                        ...
                        className={`relative p-4 rounded-2xl border cursor-pointer transition-all duration-200 flex flex-col justify-between space-y-3 ${
                          isActive
                            ? 'border-primary bg-primary/10 shadow-[0_0_15px_rgba(var(--primary-rgb),0.15)]'
                            : 'border-border hover:border-primary/40 bg-background-panel hover:bg-background-hover'
                        }`}
                      >
                        {/* Visual Color Palette Preview Box */}
                        <div 
                          className="h-24 rounded-xl p-3 flex flex-col justify-between border border-black/10 overflow-hidden relative shadow-inner"
                          style={{ backgroundColor: themeItem.previewBg }}
                        >
                          <div className="flex items-center justify-between">
                            <div className="w-14 h-3 rounded-md shadow-sm" style={{ backgroundColor: themeItem.previewPrimary }} />
                            <div className="flex space-x-1">
                              <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: themeItem.previewPrimary }} />
                              <div className="w-2.5 h-2.5 rounded-full opacity-40" style={{ backgroundColor: themeItem.previewText }} />
                            </div>
                          </div>
                          <div className="p-2 rounded-lg flex items-center space-x-2 border border-black/10 shadow-sm" style={{ backgroundColor: themeItem.previewPanel }}>
                            <div className="w-3.5 h-3.5 rounded-full shrink-0" style={{ backgroundColor: themeItem.previewPrimary }} />
                            <div className="h-2 flex-1 rounded" style={{ backgroundColor: themeItem.previewText, opacity: 0.3 }} />
                          </div>
                        </div>

                        {/* Theme Labels & Active Badge */}
                        <div className="flex items-start justify-between">
                          <div className="pr-2">
                            <h4 className="text-xs font-bold text-text flex items-center gap-1.5">
                              {t(themeItem.nameKey, lang)}
                            ...
                            <p className="text-[10px] text-text-muted mt-1 leading-relaxed">{t(themeItem.descKey, lang)}</p>
                          </div>
                          {isActive && (
                            <span className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-primary text-white shrink-0 shadow-sm">
                              {t('theme_active_badge', lang)}
                            </span>
                          )}
                        </div>
                      </div>
            ...
            <div className="space-y-6 text-left">
              ...
              {loadingMedia ? (
                ...
                <div className="space-y-6">
                  ...
                  {/* Pagination Controls */}
                  {totalPages > 1 && (
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-8 pt-4 border-t border-white/5 select-none animate-fade-in">
                      <span className="text-xs text-text-disabled">
                        {lang === 'en'
                          ? `Showing ${Math.min(filteredMediaFiles.length, (validCurrentPage - 1) * PAGE_SIZE + 1)}–${Math.min(filteredMediaFiles.length, validCurrentPage * PAGE_SIZE)} of ${filteredMediaFiles.length}`
                          : `Показано ${Math.min(filteredMediaFiles.length, (validCurrentPage - 1) * PAGE_SIZE + 1)}–${Math.min(filteredMediaFiles.length, validCurrentPage * PAGE_SIZE)} из ${filteredMediaFiles.length}`}
                      </span>
                      <div className="flex items-center space-x-1">
                        <button
                          ...
                          className="w-8 h-8 rounded-lg bg-white/[0.02] border border-white/5 text-text-muted hover:text-white disabled:opacity-30 disabled:hover:text-text-muted transition-all cursor-pointer flex items-center justify-center text-xs focus:outline-none"
                          title={lang === 'en' ? 'First Page' : 'Первая страница'}
                        >
                          «
                        </button>
                        <button
                          ...
                          className="w-8 h-8 rounded-lg bg-white/[0.02] border border-white/5 text-text-muted hover:text-white disabled:opacity-30 disabled:hover:text-text-muted transition-all cursor-pointer flex items-center justify-center text-xs focus:outline-none"
                          title={lang === 'en' ? 'Previous Page' : 'Предыдущая страница'}
                        >
                          ‹
                        </button>
                        ...
                        {(() => {
                          const pages = [];
                          const delta = 1; // Show current page + 1 page before/after
                          
                          const left = validCurrentPage - delta;
                          const right = validCurrentPage + delta + 1;
                          let range = [];
                          let l;
                          ...
                          return pages.map((page, idx) => {
                            if (page === '...') {
                              return (
                                <span key={`dots-${idx}`} className="px-1.5 text-text-disabled text-xs">
                                  ...
                                </span>
                              );
                            }
                            const isPageActive = page === validCurrentPage;
                            return (
                              <button
                                type="button"
                                key={`page-${page}`}
                                onClick={() => setCurrentPage(page as number)}
                                className={`w-8 h-8 rounded-lg border transition-all cursor-pointer text-xs font-semibold focus:outline-none ${
                                  isPageActive
                        ...
                        <button
                          ...
                          onClick={() => setCurrentPage(validCurrentPage + 1)}
                          className="w-8 h-8 rounded-lg bg-white/[0.02] border border-white/5 text-text-muted hover:text-white disabled:opacity-30 disabled:hover:text-text-muted transition-all cursor-pointer flex items-center justify-center text-xs focus:outline-none"
                          title={lang === 'en' ? 'Next Page' : 'Следующая страница'}
                        >
                          ›
                        </button>
                        <button
                          ...
                          className="w-8 h-8 rounded-lg bg-white/[0.02] border border-white/5 text-text-muted hover:text-white disabled:opacity-30 disabled:hover:text-text-muted transition-all cursor-pointer flex items-center justify-center text-xs focus:outline-none"
                          title={lang === 'en' ? 'Last Page' : 'Последняя страница'}
                        >
                          »
                        </button>
                      </div>
          ...
          ) : (activeTab === 'templates' && currentUser.role !== 'Viewer') ? (
            <div className="space-y-6 text-left">
              {templatesStatus && (
                <div className={`p-4 rounded-xl text-xs flex items-start space-x-2.5 border ${
                  templatesStatus.type === 'success' 
                    ? 'bg-green-500/10 border-green-500/20 text-green-400' 
                    : 'bg-red-500/10 border-red-500/20 text-red-400'
                }`}>
                  {templatesStatus.type === 'success' ? <Check className="w-4 h-4 shrink-0 mt-0.5" /> : <AlertTriangle className="w-4 h-4 shrink-0 mt-0.5" />}
                  <span>{templatesStatus.message}</span>
                </div>
              )}
              ...
              <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/5 space-y-4">
                <div className="flex items-center space-x-2.5 text-primary">
                  ...
                  <h3 className="text-sm font-bold text-white uppercase tracking-wider">{t('settings_templates_title', lang)}</h3>
                </div>

                {/* Инфо о пути шаблонов */}
                <div className="bg-primary/5 border border-primary/20 rounded-xl p-3 text-xs text-text-muted flex items-center space-x-2 font-sans select-none">
                  <span className="text-primary text-base">ℹ️</span>
                  <span>
                    {lang === 'en' 
                      ? `Your templates folder: ` 
                      : `Ваша папка шаблонов: `}
                    <code className="text-primary bg-primary/10 px-1.5 py-0.5 rounded font-mono font-semibold">{templatesFolder}</code>
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5 text-left font-sans">
                    <label className="block text-xs font-semibold text-text-muted">
                      {t('settings_templates_date_format', lang)}
                    </label>
                    <div className="relative font-sans">
                      <input
                        type="text"
                        value={templatesDateFormat}
                        onChange={(e) => setTemplatesDateFormat(e.target.value)}
                        className="w-full bg-black/40 border border-white/10 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-primary/60 transition-all font-sans"
                        list="date-formats"
                      />
                  ...
                  <div className="space-y-1.5 text-left font-sans">
                    <label className="block text-xs font-semibold text-text-muted font-sans">
                      {t('settings_templates_time_format', lang)}
                    </label>
                    <div className="relative font-sans">
                      <input
                        type="text"
                        value={templatesTimeFormat}
                        onChange={(e) => setTemplatesTimeFormat(e.target.value)}
                        className="w-full bg-black/40 border border-white/10 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-primary/60 transition-all font-sans"
                        list="time-formats"
                      />
                ...
                <div className="flex justify-end">
                  <button
                    onClick={handleSaveTemplatesSettings}
                    className="px-4 py-2 bg-primary hover:bg-primary-hover text-white text-xs font-semibold rounded-xl flex items-center space-x-1.5 shadow-glow hover:scale-[1.02] active:scale-95 transition-all cursor-pointer font-sans"
                  >
                    <Check className="w-3.5 h-3.5" />
                    <span>{t('settings_btn_save', lang) || 'Save'}</span>
                  </button>
                </div>
              </div>
                ...
                <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/5 space-y-4">
                  <div className="flex justify-between items-center">
                    <div className="flex items-center space-x-2 text-primary">
                      ...
                      <h3 className="text-sm font-bold text-white uppercase tracking-wider">{t('settings_templates_list_title', lang)}</h3>
                    </div>
                    <button
                      onClick={handleCreateTemplate}
                      className="px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-primary/30 text-white text-xs font-medium rounded-xl flex items-center space-x-1.5 transition-all hover:scale-[1.01] active:scale-95 cursor-pointer font-sans"
                    >
                      <UserPlus className="w-3.5 h-3.5 text-primary shrink-0" />
                      <span>{t('settings_templates_btn_create', lang)}</span>
                    </button>
                  </div>

                  {templatesList.length === 0 ? (
                    <div className="text-center py-6 text-xs text-text-disabled bg-black/10 border border-dashed border-white/5 rounded-xl font-sans">
                      {t('template_modal_no_templates', lang, { folder: templatesFolder })}
                    </div>
                  ) : (
                    <div className="border border-white/5 rounded-2xl overflow-hidden bg-black/20 font-sans">
                      <table className="w-full text-left border-collapse text-xs">
                        <thead>
                          <tr className="border-b border-white/5 bg-black/40 text-text-muted">
                        ...
                        <tbody>
                          {templatesList.map(template => (
                            <tr key={template.relative_path} className="border-b border-white/5 hover:bg-white/[0.02] text-white">
                              <td className="p-3 truncate font-semibold font-sans">{template.title}</td>
                              <td className="p-3 text-text-disabled truncate font-sans">{template.relative_path}</td>
                              <td className="p-3 text-right">
                                <div className="flex justify-end items-center space-x-2">
                                  <button
                                    onClick={() => {
                                      if (onOpenNote) {
                                        onOpenNote(template.relative_path);
                                      }
                                      onClose();
                                    }}
                                    className="px-2.5 py-1 hover:bg-primary/20 hover:text-white border border-white/5 hover:border-primary/20 text-text-muted rounded-lg text-[10px] font-medium transition-all cursor-pointer flex items-center space-x-1 font-sans"
                                    title={t('settings_templates_btn_edit', lang)}
                                  >
                                    <Edit2 className="w-3.5 h-3.5" />
                                    <span>{t('settings_templates_btn_edit', lang)}</span>
                                  </button>
                                  <button
                                    onClick={async () => {
                                      if (!confirm(t('settings_templates_confirm_delete', lang, { name: template.title }))) return;
                                      try {
                                        const res = await fetch(`/api/notes?relative_path=${encodeURIComponent(template.relative_path)}`, {
                                          method: 'DELETE',
                                          headers: { 'Authorization': `Bearer ${token}` }
                                        });
                                        if (!res.ok) throw new Error();
                                        ...
                                        alert(t('sidebar_delete_failed', lang));
                                      }
                                    }}
                                    className="px-2.5 py-1 hover:bg-red-500/20 text-text-muted hover:text-red-400 border border-white/5 hover:border-red-500/20 rounded-lg text-[10px] font-medium transition-all cursor-pointer flex items-center space-x-1 font-sans"
                                    title={t('settings_templates_btn_delete', lang)}
                                  >
                                    <Trash2 className="w-3.5 h-3.5" />
                                    <span>{t('settings_templates_btn_delete', lang)}</span>
                                  </button>
                                </div>
                              </td>
                            </tr>
