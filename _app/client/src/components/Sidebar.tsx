/**
 * @module Sidebar
 * Left explorer menu tree, active user indicator list, and notification stack drawer.
 */
...
interface SidebarProps {
  ...
  onChangeOwner?: (item: Note) => void;
}

/**
 * Left navigation sidebar component.
 * Renders the folder explorer tree, new file/folder actions, active users presence list,
 * global notifications list (with comment and suggestion filters), and footer controls.
 */
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const handleCycleTheme = () => {
    ...
    setCurrentTheme(isLight ? 'dark-purple' : 'light-purple');
  };
  const [activeTab, setActiveTab] = useState<'notes' | 'tags'>('notes');
  const [tags, setTags] = useState<{ name: string; count: number }[]>([]);
  const [tagsFilter, setTagsFilter] = useState('');

  // Fetch tags from API when Tags tab is active or notes change
  useEffect(() => {
    if (activeTab === 'tags') {
  ...
  const filteredTags = useMemo(() => {
    if (!tagsFilter) return tags;
    const query = tagsFilter.toLowerCase();
  ...
  // Filter out the templates folder and its contents from the file explorer
  const visibleNotes = useMemo(() => {
    if (!templatesFolder) return notes;
    const folderPrefix = templatesFolder.endsWith('/') ? templatesFolder : templatesFolder + '/';
    return notes.filter(n => 
      n.relative_path !== templatesFolder && 
      !n.relative_path.startsWith(folderPrefix)
    );
  }, [notes, templatesFolder]);

  // Expand all folders in the file tree
  const expandAllFolders = () => {
    const folders = visibleNotes.filter(n => n.is_directory);
  ...
  const filteredNotes = useMemo(() => {
    if (!searchQuery) return visibleNotes;
    const query = searchQuery.toLowerCase();
    
    // Find all notes/folders that match the query
    const matched = visibleNotes.filter(n => n.title.toLowerCase().includes(query));
              ...
              <li key={item.relative_path} className="group/dir">
                ...
                >
                  ...
                  {currentUser.role !== 'Viewer' && (
                    <div className="flex items-center space-x-0.5 opacity-0 group-hover/dir:opacity-100 transition-all shrink-0">
                      {currentUser.role === 'Admin' && onChangeOwner && (
                        <button
                          ...
                          className="p-1 text-text-muted hover:text-white hover:bg-white/10 rounded cursor-pointer transition-all"
                          title={t('change_owner_menu', lang)}
                        >
                      ...
                      <button
                        ...
                        className="p-1 text-text-muted hover:text-red-400 hover:bg-red-500/20 rounded cursor-pointer transition-all"
                ...
                {isExpanded && (
                  <div className="border-l border-primary/15 ml-3">
            ...
            return (
              <li key={item.relative_path} className="group/file">
                ...
                >
                  ...
                  {currentUser.role !== 'Viewer' && (
                    <div className="flex items-center space-x-0.5 opacity-0 group-hover/file:opacity-100 transition-all shrink-0">
                      {currentUser.role === 'Admin' && onChangeOwner && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                      ...
                      <button
                        ...
                        className={`p-1 rounded cursor-pointer transition-all ${
                          isActiveFile 
                            ...
                            : 'text-text-muted hover:text-red-400 hover:bg-red-500/20'
                        }`}
  ...
  return (
    <div className="flex flex-col h-full w-full bg-background-panel border-r border-border overflow-visible text-left select-none">
      
      {/* User Info Header */}
      <div className="p-4 border-b border-border bg-background-panel flex items-center justify-between">
        <div className="flex items-center space-x-2.5 min-w-0">
          ...
          <div className="min-w-0">
            <h3 className="text-xs font-bold text-text truncate">{currentUser.username}</h3>
            <span className="text-[10px] text-primary uppercase font-extrabold tracking-wider">{currentUser.role}</span>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-1.5">
          ...
          {(() => {
            ...
            <div className="relative">
              <button
                ...
                className={`p-1.5 hover:bg-white/5 rounded-lg transition-colors cursor-pointer relative focus:outline-none ${
          ...
          <button
            ...
            className="p-1.5 hover:bg-background-hover hover:text-primary text-text-muted rounded-lg transition-colors cursor-pointer focus:outline-none"
            title={currentTheme.startsWith('light') ? (lang === 'en' ? 'Switch to Dark Theme' : 'Переключить на тёмную тему') : (lang === 'en' ? 'Switch to Light Theme' : 'Переключить на светлую тему')}
          >
          ...
          <button
            onClick={onLogout}
            className="p-1.5 hover:bg-background-hover hover:text-red-400 text-text-muted rounded-lg transition-colors cursor-pointer focus:outline-none"
      ...
      {/* Creation Tools */}
      {currentUser.role !== 'Viewer' && (
        <div className="p-3 grid grid-cols-2 gap-2 border-b border-primary/10">
          <button
            onClick={() => handleCreate(false)}
            className="flex items-center justify-center space-x-1 py-1.5 bg-white/5 hover:bg-white/10 active:scale-95 text-xs text-text rounded-lg transition-all cursor-pointer border border-white/5"
          >
      ...
      {/* Tabs Switcher */}
      <div className="px-3 pb-2 flex space-x-1 border-b border-primary/10 pt-1">
        <button
          onClick={() => setActiveTab('notes')}
          className={`flex-1 py-1.5 rounded-lg text-xs font-semibold flex items-center justify-center space-x-1.5 transition-all cursor-pointer ${
            activeTab === 'notes'
              ? 'bg-primary/20 text-white font-semibold border border-primary/40 shadow-sm'
              : 'text-text-muted hover:bg-primary/10 hover:text-text border border-transparent'
          }`}
        >
          <Folder className={`w-3.5 h-3.5 ${activeTab === 'notes' ? 'text-primary' : 'text-primary'}`} />
          <span>{t('tab_notes', lang)}</span>
        </button>
        <button
        ...
        >
          <span className={`font-extrabold text-xs ${activeTab === 'tags' ? 'text-primary' : 'text-primary'}`}>#</span>
          <span>{t('tab_tags', lang)}</span>
        </button>
      </div>
      ...
      <div className="px-3 py-2 flex items-center space-x-2">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder={activeTab === 'notes' ? t('sidebar_search_placeholder', lang) : t('tags_search_placeholder', lang)}
            value={activeTab === 'notes' ? searchQuery : tagsFilter}
            onChange={(e) => activeTab === 'notes' ? setSearchQuery(e.target.value) : setTagsFilter(e.target.value)}
            className="w-full pl-9 pr-4 py-1.5 bg-primary/5 border border-primary/15 rounded-lg text-xs text-text placeholder:text-text-disabled focus:outline-none focus:border-primary/40 transition-colors"
          />
        </div>
        {onOpenSearch && (
          <button
            onClick={() => onOpenSearch(activeTab === 'tags' && tagsFilter ? `#${tagsFilter}` : '')}
            className="p-1.5 bg-primary/10 border border-primary/20 rounded-lg text-primary hover:text-white hover:bg-primary/30 transition-all cursor-pointer flex items-center justify-center shrink-0"
      ...
      {/* Tab Contents: Notes Tree Explorer or Tags Explorer */}
      {activeTab === 'notes' ? (
            ...
            <div className="grid grid-cols-1 gap-1">
              {filteredTags.map(tag => (
                <button
                  ...
                  onClick={() => {
                    if (onOpenSearch) {
                      onOpenSearch(`#${tag.name}`);
                    }
                  }}
                  className="flex items-center justify-between px-2.5 py-1.5 rounded-lg text-xs hover:bg-white/5 text-left text-text-muted hover:text-white transition-all cursor-pointer border border-white/5 hover:border-primary/30 bg-black/10 hover:bg-black/20"
                >
                  <span className="font-semibold text-primary truncate">#{tag.name}</span>
                  <span className="bg-primary/20 text-primary text-[10px] px-2 py-0.5 rounded-full font-bold select-none border border-primary/10 shadow-sm shrink-0">
                    {tag.count}
                  </span>
                </button>
              ))}
      ...
      <div className="p-3 border-t border-primary/10 flex flex-col space-y-2">
        <button
          onClick={onOpenExport}
          className="w-full py-2 bg-primary hover:bg-primary-hover active:scale-[0.98] text-white text-xs font-semibold rounded-lg flex items-center justify-center space-x-2 transition-all cursor-pointer shadow-sm"
        >
