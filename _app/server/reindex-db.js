...
export async function reindexDatabase(force = false, onProgress = null) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    // Also update FTS index for this file to keep FTS index synchronized
    try {
      const title = note.relative_path.split('/').pop().replace('.md', '');
      ...
      console.error(`[Reindex DB] Failed to update FTS for ${note.relative_path}:`, ftsErr);
    }
