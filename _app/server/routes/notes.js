...
import { vaultPath, extractTags } from '../watcher.js';
 ...
 */
export const updateNoteTags = async (relPath, tags) => {
  try {
    ...
    if (tags.length > 0) {
      console.log(`[Tags] Successfully updated tags for: ${relPath} (${tags.join(', ')})`);
    }
  } catch (err) {
    console.error(`[Tags] Failed to update tags for ${relPath}:`, err);
  }
};
...
router.get('/content', authenticateJWT, async (req, res) => {
  ...
  if (!checkTemplateAccess(relPath, req.user)) {
...
router.put('/', authenticateJWT, canEdit, checkLock, async (req, res) => {
  ...
  try {
    ...
    updateNoteTags(normPath, tags).catch(err => {
      console.error('[Tags] Background update failed:', err);
    });
...
// 4.5. Update note or folder owner (created_by) - Admin only
router.put('/owner', authenticateJWT, checkAdmin, async (req, res) => {
  const { relative_path, new_owner, recursive } = req.body;
  if (!relative_path) return res.status(400).json({ error: 'relative_path is required' });
  if (!new_owner) return res.status(400).json({ error: 'new_owner is required' });
  ...
  try {
    ...
    // Check if new_owner exists in users table or is system default
    if (new_owner !== 'Внешняя система' && new_owner !== 'system') {
      const userExists = await get('SELECT id FROM users WHERE username = ?', [new_owner]);
      if (!userExists) {
    ...
    if (note && note.is_directory && recursive) {
      const result = await run(
        'UPDATE notes SET created_by = ? WHERE relative_path = ? OR relative_path LIKE ?',
        [new_owner, normPath, normPath + '/%']
      );
      updatedCount = result.changes || 0;
    } else {
      const result = await run(
        'UPDATE notes SET created_by = ? WHERE relative_path = ?',
        [new_owner, normPath]
      );
      updatedCount = result.changes || 0;
    }

    req.app.get('io').emit('notes:updated');
    req.app.get('io').emit('note:owner_changed', { relative_path: normPath, created_by: new_owner, recursive });
    ...
    console.error(`Error updating owner for ${normPath}:`, err);
    res.status(500).json({ error: 'Failed to update document owner' });
  }
});
...
// 6.4. Get All Tags with counts
router.get('/tags', authenticateJWT, async (req, res) => {
  try {
    const tags = await all('SELECT tag, COUNT(*) as count FROM note_tags GROUP BY tag ORDER BY tag ASC');
    res.json(tags.map(t => ({ name: t.tag, count: t.count })));
  } catch (err) {
...
router.get('/graph-data', authenticateJWT, async (req, res) => {
  try {
    let notesList = await all('SELECT relative_path, title, is_directory FROM notes WHERE is_directory = 0');
    
    // Exclude system templates folder from graph
    notesList = notesList.filter(n => !n.relative_path.startsWith('Templates/'));

    const links = [];
    const validNotes = [];
    const seenLinks = new Set(); // For deduplicating wiki links
        ...
        while ((match = wikiLinkRegex.exec(content)) !== null) {
          ...
          if (targetNote && targetNote.relative_path !== note.relative_path) {
            ...
            if (fs.existsSync(targetAbsolutePath)) {
              ...
              const linkKey = `${note.relative_path}->${targetNote.relative_path}`;
    ...
    // Load Tags & Tag links
    const dbNoteTags = await all('SELECT relative_path, tag FROM note_tags');
    const validNotePaths = new Set(validNotes.map(n => n.relative_path));
    const uniqueTags = new Set();
    const tagLinks = [];

    for (const item of dbNoteTags) {
      if (validNotePaths.has(item.relative_path)) {
    ...
    const tagNodes = Array.from(uniqueTags).map(tag => ({
      id: `tag:${tag}`,
      name: `#${tag}`,
      isTag: true,
      val: 1.2
    ...
    res.json({
      nodes: [
        ...validNotes.map(n => ({ id: n.relative_path, name: n.title, val: 1 })),
        ...tagNodes
      ],
      links: [
        ...links,
...
// Helper to check if path starts with any excluded folder
const isPathExcluded = (relPath, excludedList) => {
  if (!excludedList || excludedList.length === 0) return false;
  const parts = relPath.split('/');
  const rootFolder = parts.length > 1 ? parts[0] : '__root__';
  return excludedList.includes(rootFolder);
};

// 9. Global Search (Full-Text, Semantic AI, Title)
router.get('/search', authenticateJWT, async (req, res) => {
  const { q, mode = 'fts', exclude = '' } = req.query;
  if (!q) {
    return res.status(400).json({ error: 'Query parameter q is required' });
  }

  const excludedList = exclude ? exclude.split(',') : [];

  // Exclude system templates folder from search
  excludedList.push('Templates');

  try {
    if (mode === 'title') {
      const results = await all(
        'SELECT relative_path, title FROM notes WHERE is_directory = 0 AND (title LIKE ? OR relative_path LIKE ?) LIMIT 100',
        [`%${q}%`, `%${q}%`]
      );
      const filtered = results
        .filter(r => !isPathExcluded(r.relative_path, excludedList))
        .map(r => ({ relative_path: r.relative_path, title: r.title, score: 1 }))
        .slice(0, 20);
    ...
    if (mode === 'semantic') {
      ...
      for (const item of allEmbeddings) {
        if (isPathExcluded(item.relative_path, excludedList)) {
          continue;
        }
        try {
    ...
    if (q.startsWith('#')) {
      const tagPart = q.slice(1).trim();
      if (!tagPart) {
        return res.json([]);
      }

      const tagResults = await all(`
        SELECT DISTINCT n.relative_path, n.title, nt.tag
        FROM note_tags nt
        JOIN notes n ON nt.relative_path = n.relative_path
        WHERE nt.tag LIKE ? AND n.is_directory = 0
        LIMIT 100
      `, [`%${tagPart}%`]);
      ...
      tagResults.sort((a, b) => {
        ...
        const queryLower = tagPart.toLowerCase();
      ...
      const filtered = tagResults
        .filter(r => !isPathExcluded(r.relative_path, excludedList))
        .map(r => ({
          relative_path: r.relative_path,
          title: r.title,
          snippet: `<span class="note-tag cursor-pointer hover:bg-cyan-500/20 px-1.5 py-0.5 rounded border border-cyan-500/10 text-cyan-400 font-medium inline-block align-middle select-none transition-all mr-1 my-0.5 text-[11px]" data-tag="${r.tag}">#${r.tag}</span>`,
          score: 1.0
        }))
        .slice(0, 20);
    ...
    const ftsResults = await all(`
      SELECT 
        relative_path, 
        title, 
        snippet(notes_fts, 2, '[MARK_START]', '[MARK_END]', '...', 15) as snippet,
    ...
    const filtered = ftsResults
      ...
      .map(r => {
        ...
        if (r.snippet) {
          ...
          safeSnippet = escaped
            .replace(/\[MARK_START\]/g, '<mark class="bg-primary/30 text-white font-bold px-0.5 rounded">')
    ...
    try {
      ...
      const filtered = results
        .filter(r => !isPathExcluded(r.relative_path, excludedList))
        .map(r => ({ relative_path: r.relative_path, title: r.title, score: 0.5 }))
        .slice(0, 20);
      return res.json(filtered);
    } catch (fallbackErr) {
