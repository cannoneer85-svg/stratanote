...
router.post('/push', authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    let serverFileHash = '';
    if (serverFileExists) {
      serverFileHash = await getFileHash(safePath);
    }
    ...
    const newHash = await getFileHash(safePath);
    const stat = fs.statSync(safePath);
    fileHashCache.set(safePath, { hash: newHash, mtime: stat.mtimeMs, size: stat.size });
    saveHashCache();
