...
/**
 * Helper to purge orphan notes from database that no longer exist on physical disk in vaultPath.
 * Checks all indexed notes against disk and deletes orphaned records from notes, notes_fts,
 * note_embeddings, and note_tags tables.
 * @returns {Promise<number>} Number of purged orphan notes
 * @example
 * const purged = await purgeOrphanNotes();
 * console.log(`Purged ${purged} orphan notes`);
 */
export const purgeOrphanNotes = async () => {
  try {
    const dbNotes = await all('SELECT relative_path FROM notes');
    let purgedCount = 0;
    for (const note of dbNotes) {
      ...
      if (!fs.existsSync(absPath)) {
        ...
        await run('DELETE FROM note_embeddings WHERE relative_path = ?', [note.relative_path]);
        await run('DELETE FROM note_tags WHERE relative_path = ?', [note.relative_path]);
        purgedCount++;
      }
    }
    if (purgedCount > 0) {
      console.log(`[Watcher] Purged ${purgedCount} orphan notes from database that were missing on disk.`);
    }
    return purgedCount;
  } catch (err) {
    console.error('[Watcher] Failed to purge orphan notes:', err);
    return 0;
  }
};

/**
 * Initializes Chokidar file watcher to monitor vault directory for real-time changes.
 * @param {Object} io - Socket.io server instance for broadcasting events
