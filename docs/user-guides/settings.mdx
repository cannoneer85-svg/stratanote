...
* **Access Control:** Non-admin users can strictly only view, edit, or comment on templates inside their own personal directory. Accessing other users' directories via direct URL hashes or API requests is blocked by the backend, returning `403 Forbidden` and gracefully returning the client back to safety.
